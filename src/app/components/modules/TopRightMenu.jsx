...
function TopRightMenu({username, showLogin, logout, loggedIn, vertical, navigate, toggleOffCanvasMenu, probablyLoggedIn}) {
    ...
    if (loggedIn) { // change back to if(username) after bug fix:  Clicking on Login does not cause drop-down to close #TEMP!
        ...
        return (
            <ul className={mcn + mcl}>
                <li className={lcn + " Header__search"}><a href="https://www.asksteem.com/" title={tt_search}>{vertical ? <span>{tt_search}</span> : <Icon name="search" />}</a></li>
                {submit_story}
    ...
    if (probablyLoggedIn) {
        return (
            <ul className={mcn + mcl}>
                {!vertical && <li className="Header__search"><a href="https://www.asksteem.com/" title={tt_search}><Icon name="search" /></a></li>}
